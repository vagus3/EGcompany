...
const nextConfig: NextConfig = {
  ...
  // sharp/resvg는 네이티브 바이너리를 포함하는 패키지라 번들링하지 않고
  // node_modules에서 그대로 로드해야 한다. 번들링 시 바이너리가 누락되어
  // "Could not load the sharp module using the linux-x64 runtime" 에러 발생.
  serverExternalPackages: ["sharp", "@resvg/resvg-js"],
  // 서버리스 함수 빌드 시 fs로 직접 읽는 사원증 템플릿/폰트 파일과,
  // serverExternalPackages만으로는 누락되는 네이티브 바이너리를 명시적으로
  // 포함시킨다. pnpm-lock.yaml에 고정된 버전 기준 경로.
  outputFileTracingIncludes: {
    "/api/terminal/completion": [
      ...
      "./node_modules/.pnpm/@resvg+resvg-js-linux-x64-gnu@2.6.2/node_modules/@resvg/resvg-js-linux-x64-gnu/**/*",
    ],
  },
};
